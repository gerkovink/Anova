...
evaluate <- function(sim){
  ...
  fit.imp <- with(data = sim$imp, expr = lm(y ~ x)) # Use with workflow to calculate D3 later
  ...
  med.p.imp <- median(unlist(with(sim$imp, anova(lm(y ~ x))$'Pr(>F)'[1])$analyses))
...
# MAR sims
MAR_EVAL01 <- map(MAR_SIM01, evaluate)
MAR_EVAL25 <- map(MAR_SIM25, evaluate)
MAR_EVAL50 <- map(MAR_SIM50, evaluate)
MAR_EVAL75 <- map(MAR_SIM75, evaluate)
...
MAR_F.out01 <- mapply(grab.F, MAR_EVAL01, SIMPLIFY = FALSE) %>% do.call(rbind, .)
MAR_F.out25 <- mapply(grab.F, MAR_EVAL25, SIMPLIFY = FALSE) %>% do.call(rbind, .)
MAR_F.out50 <- mapply(grab.F, MAR_EVAL50, SIMPLIFY = FALSE) %>% do.call(rbind, .)
MAR_F.out75 <- mapply(grab.F, MAR_EVAL75, SIMPLIFY = FALSE) %>% do.call(rbind, .)
...
MAR_p.out01 <- mapply(grab.p, MAR_EVAL01, SIMPLIFY = FALSE) %>% do.call(rbind, .)
MAR_p.out25 <- mapply(grab.p, MAR_EVAL25, SIMPLIFY = FALSE) %>% do.call(rbind, .)
MAR_p.out50 <- mapply(grab.p, MAR_EVAL50, SIMPLIFY = FALSE) %>% do.call(rbind, .)
MAR_p.out75 <- mapply(grab.p, MAR_EVAL75, SIMPLIFY = FALSE) %>% do.call(rbind, .)

# Save the evaluations and the resulting p-value and F-value objects
save(list = c("EVAL01", "EVAL25", "EVAL50", "EVAL75", "MAR_EVAL01", "MAR_EVAL25", "MAR_EVAL50", "MAR_EVAL75"), 
     file = "Workspaces/Evaluations_objects.RData")
save(list = c("F.out01", "F.out25", "F.out50", "F.out75", 
              "p.out01", "p.out25", "p.out50", "p.out75", 
              "MAR_F.out01", "MAR_F.out25", "MAR_F.out50", "MAR_F.out75", 
              "MAR_p.out01", "MAR_p.out25", "MAR_p.out50", "MAR_p.out75"), 
